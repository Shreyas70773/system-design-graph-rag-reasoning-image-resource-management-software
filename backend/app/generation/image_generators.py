...
class ReplicateGenerator(ImageGenerator):
    ...
    async def _run_prediction(self, model: str, input_data: dict) -> dict:
        ...
        # Replicate supports two creation styles:
        # 1) /predictions with a version hash
        # 2) /models/{owner}/{name}/predictions with latest model
        if ":" in model:
...
class FallbackGenerator(ImageGenerator):
    ...
    Priority order is config-driven and diffusion-first by default.
    Google/OpenRouter image models are disabled unless explicitly enabled in settings.
    """
    ...
    def __init__(self):
        ...
        allow_openrouter = bool(settings.allow_openrouter_image_models)
        allow_google = bool(settings.allow_google_image_models)

        # Build list of available generators - ORDER MATTERS (first = tried first)
        provider_priority_raw = settings.image_provider_priority or "fal.ai,replicate"
        provider_priority = [
            item.strip().lower()
            for item in provider_priority_raw.split(",")
            if item and item.strip()
        ]

        if not provider_priority:
            provider_priority = ["fal.ai", "replicate"]

        for provider_name in provider_priority:
            if provider_name == "fal.ai" and os.getenv("FAL_KEY"):
                self.generators.append(("fal.ai", FalAIGenerator()))
            elif provider_name == "replicate" and os.getenv("REPLICATE_API_TOKEN"):
                self.generators.append(("replicate", ReplicateGenerator()))
            elif provider_name == "openrouter" and allow_openrouter and os.getenv("OPENROUTER_API_KEY"):
                self.generators.append(("openrouter", OpenRouterGenerator()))
            elif provider_name == "gemini" and allow_google and os.getenv("GOOGLE_API_KEY"):
                self.generators.append(("gemini", GeminiGenerator()))

        if not self.generators:
            print(
                "WARNING: No image generators configured. "
                "Set FAL_KEY/REPLICATE_API_TOKEN or enable Google/OpenRouter providers in settings."
            )
