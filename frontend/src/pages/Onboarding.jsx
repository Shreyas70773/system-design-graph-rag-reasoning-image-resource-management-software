...
export default function Onboarding() {
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      if (data.logo?.url && data.id) {
        try {
          const quality = await checkLogoQuality(data.id)
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      for (const color of (brandData.colors || [])) {
        try {
          await addBrandColor(brandData.id, {
            hex: color.hex,
      ...
      for (const product of products) {
        try {
          await addBrandProduct(brandData.id, {
            name: product.name,
      ...
      for (const char of characters) {
        try {
          await addBrandCharacter(brandData.id, {
            name: char.name,
      ...
      navigate(`/generate/${brandData.id}`, {
        state: {
          fontId: selectedFont,
