...
/**
 * Run a controlled research generation for one method across multiple seeds.
 * @param {Object} payload - Controlled generation payload
 * @returns {Promise<Object>} Run summary with candidate outputs and aggregated metrics
 */
export const runResearchControlledGeneration = async (payload) => {
  const response = await api.post('/api/research/generate-controlled', payload);
  return response.data;
};

/**
 * Run a shared-experiment ablation sweep.
 * @param {Object} payload - Ablation request payload
 * @returns {Promise<Object>} Collection of runs under one experiment ID
 */
export const runResearchAblation = async (payload) => {
  const response = await api.post('/api/research/run-ablation', payload);
  return response.data;
};
 ...
 * @returns {Promise<Object>} Run metadata and metric snapshots
 */
export const getResearchRunMetrics = async (runId) => {
  const response = await api.get(`/api/research/metrics/${runId}`);
  return response.data;
};

/**
 * Get locked experiment manifest.
 * @param {string} experimentId - Experiment ID
 * @returns {Promise<Object>} Manifest payload
 */
export const getResearchManifest = async (experimentId) => {
  const response = await api.get(`/api/research/manifest/${experimentId}`);
  return response.data;
};

/**
 * Validate a requested manifest against an existing experiment lock.
 * @param {Object} payload - Manifest validation payload
 * @returns {Promise<Object>} Validation diagnostics
 */
export const validateResearchManifest = async (payload) => {
  const response = await api.post('/api/research/manifest/validate', payload);
  return response.data;
};

/**
 * Compare all runs under a shared experiment ID.
 * @param {string} experimentId - Experiment ID
 * @returns {Promise<Object>} Comparison payload
 */
export const compareResearchExperiment = async (experimentId) => {
  const response = await api.get(`/api/research/compare/${experimentId}`);
  return response.data;
};

/**
 * Run paired statistical comparison for a metric in one experiment.
 * @param {string} experimentId - Experiment ID
 * @param {string} metric - Metric key (e.g. brand_score, color_alignment_score)
 * @param {string} baselineMethod - Baseline method key
 * @param {Object} options - Optional analysis controls
 * @param {number} options.bootstrapResamples - Bootstrap resample count for CI
 * @param {number} options.ciAlpha - Two-sided alpha for confidence interval bounds
 * @param {number} options.randomSeed - Stable seed for bootstrap sampling
 * @returns {Promise<Object>} Statistical comparison payload
 */
export const getResearchStats = async (
  experimentId,
  metric = 'brand_score',
  baselineMethod = 'prompt_only',
  options = {}
) => {
  const {
  ...
  const response = await api.get(`/api/research/stats/${experimentId}`, {
    params: {
      metric,
...
/**
 * Trigger post-run Lab-space DeltaE refinement job.
 * @param {string} runId - Run ID
 * @returns {Promise<Object>} Job summary
 */
export const runResearchDeltaEJob = async (runId) => {
  const response = await api.post(`/api/research/jobs/deltae/${runId}`);
  return response.data;
};

/**
 * Export one run in JSON or CSV.
 * @param {string} runId - Run ID
 * @param {'json'|'csv'} format - Export format
 * @returns {Promise<Object|Blob>} Export payload
 */
export const exportResearchRun = async (runId, format = 'json') => {
  const response = await api.get(`/api/research/export/run/${runId}`, {
    params: { format },
...
/**
 * Export experiment comparison table in JSON or CSV.
 * @param {string} experimentId - Experiment ID
 * @param {'json'|'csv'} format - Export format
 * @returns {Promise<Object|Blob>} Export payload
 */
export const exportResearchExperiment = async (experimentId, format = 'json') => {
  const response = await api.get(`/api/research/export/experiment/${experimentId}`, {
    params: { format },
    responseType: format === 'csv' ? 'blob' : 'json',
  });
  return response.data;
 ...
 * @returns {Promise<Object>} Recent runs
 */
export const listResearchRuns = async (brandId, limit = 20) => {
  const response = await api.get(`/api/research/runs/${brandId}`, {
    params: { limit },
  });
  return response.data;
};
